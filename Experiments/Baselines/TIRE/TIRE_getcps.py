...
def main():    
    ...
    for i in range(timeseries.shape[1]):
        data = timeseries[:,i]
        start = time.time()
        windows = utils.ts_to_windows(data, 0, window_size, stride=20)
    ...
    np.savetxt(cps_filename+"_change_points.csv", change_points, delimiter=",")
    np.savetxt(cps_filename+"_disses.csv", disses, delimiter=",")
    data = np.loadtxt(cps_filename+"_change_points.csv", delimiter=",") 
    
    actual_cps = []
    for d in change_points:
        actual_cps.append(np.where(d > 0.5))
    flat_change_points = []
    for row in actual_cps:
        for cp in row:
            for c in cp:
