...
if string  == 'PAMAP2':
    ...
    window_length2_list = [90, 120, 150, 180, 210]
    ...
    window_length2_list = [120, 160, 200, 240, 280]
...
for window_length2 in window_length2_list:
    ...
    for loop in range(5):
        ...
        if trained == 0:
            ...
            torch.save(deep_cluster_model.state_dict(), './'+string+'_w2_pretrain.pth')
            trained = 1
        else:
            deep_cluster_model = DEC(n_clusters = n_clusters, input_shape=input_shape,k_sizes=k_sizes,strides=strides)
            deep_cluster_model.load_state_dict(torch.load('./'+string+'_w2_pretrain.pth'))
            deep_cluster_model.to(device)
