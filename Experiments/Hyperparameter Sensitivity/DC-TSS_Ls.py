...
if string  == 'PAMAP2':
    ...
    prominence = 0.08
    height = 0.5
    k_sizes = [[6,4,4],[6,4,4],[10,6,6],[10,6,6],[10,6,6]]
    strides = [[2,2,2],[3,2,2],[3,3,3],[3,3,3],[3,3,3]]
    window_length_list = [25,50,100,200,400]
    prominence = 0.05
    height = 0.5
    ...
    k_sizes = [[6,4,4],[10,8,5],[10,8,5],[10,8,5],[10,8,5]]
    strides = [[2,2,2],[3,3,2],[3,3,3],[3,3,3],[3,3,3]]
    window_length_list = [40,80,160,320,640]
    ...
    k_sizes = [[6,4,4],[30,10,10],[30,10,10],[30,10,10],[30,10,10]]
    strides = [[3,3,2],[3,3,3],[3,3,3],[3,3,3],[3,3,3]]
    window_length_list = [80,170,340,680,1300]
    batch_size = 1024
    epochs1 = 1024
    epochs2 = 8000
    update_interval = 1
    ...
    k_sizes = [[3,2],[4,4],[6,4,4],[6,4,4], [6,4,4]]
    strides = [[2,2],[2,2],[2,2,2],[2,2,2],[2,2,2]]
    window_length_list = [7,13,25,50,100]
    batch_size = 1024
    epochs1 = 1200
    epochs2 = 8000
    update_interval = 1
...
f1_n_clusters = []
trained = 0
for b,window_length in enumerate(window_length_list):
    f1s = []
    coverings = []
    trained = 0
    for loop in range(5):
        ...
        if trained == 0:
            deep_cluster_model = DEC(n_clusters = n_clusters, input_shape=input_shape,k_sizes=k_sizes[b],strides=strides[b])
            ...
            torch.save(deep_cluster_model.state_dict(), './'+string+'_sublength_pretrain.pth')
            trained = 1
        else:
            deep_cluster_model = DEC(n_clusters = n_clusters, input_shape=input_shape,k_sizes=k_sizes[b],strides=strides[b])
            deep_cluster_model.load_state_dict(torch.load('./'+string+'_sublength_pretrain.pth'))
            deep_cluster_model.to(device)
